...
- 메인 게이지 위치는 X/Y ±1000 슬라이더와 직접 숫자 입력으로 조절할 수 있으며, 직접 입력은 화면 밖 위치도 허용합니다.
- 속성 문구는 화면 중앙 기준 위치, 25~500% 크기, 여러 줄 기준점(상단/중앙/하단)을 체력 텍스트와 독립적으로 설정할 수 있습니다.
- 배율 토큰도 별도 위치와 25~500% 크기를 지원합니다. `Attach rate token to main gauge`를 켜면 위치 설정을 사용하지 않고 메인 게이지를 따라갑니다.
