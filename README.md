...
- 메인 게이지 위치는 X `-1000~1000`, Y `-500~1400` 슬라이더와 직접 숫자 입력으로 조절할 수 있습니다. 직접 입력은 슬라이더와 화면 범위를 넘어갈 수 있습니다.
- 체력 텍스트, 속성 문구, 배율 토큰은 기본적으로 0.2.0 방식으로 게이지 위에 자동 정렬됩니다. 각 항목의 `Independent position`을 켜면 화면 중앙 기준 X/Y를 직접 설정할 수 있습니다.
- 체력 텍스트 위치 슬라이더는 X/Y `-800~800`이며, 속성 문구와 배율 토큰은 각각 독립적인 25~500% 크기를 지원합니다. 속성 문구 기본 크기는 300%, 증감률/상한 토큰 기본 크기는 200%이며 각 위치 초기화 버튼은 크기도 이 기본값으로 복구합니다.
- PlanetGauge HUD는 게임의 입력 계기판 설정과 독립적으로 표시됩니다. 입력 계기판을 꺼도 메인 게이지, 커스텀 스킨, 체력·속성·증감률 텍스트와 ForceRecovery 표시가 유지됩니다.
- 기본값은 Shift+F3이며, GUI 창의 `Debug shortcut`에서 키 1+키 2 조합을 바꿀 수 있습니다.
...
- 인게임에서 게이지 온오프 아이콘도 같이 체력이 변화합니다. Blindfolded가 공개되기 전에는 메인 게이지와 아이콘 모두 체력과 분리된 검은색 고정 바로 표시됩니다.
- 커스텀 게이지 스킨은 Blindfolded 동안 체력 PNG의 전체 실루엣을 검은색으로 표시해 현재 채움량을 숨깁니다. 프레임 PNG는 원래 색상을 유지합니다.
