...
> - 체력 표시 차단: Blindfolded (게이지를 검게 바꾸고 체력을 `???`로 표시합니다. 사망 처리 시 숫자를 공개하고, 레벨 클리어 시 속성 자체를 해제합니다.)
...
- 게이지 모드, 게임 자체 실패 방지, `No-Fail Disabled`를 함께 켠 상태에서 Fail 판정이 발생하면 게임은 계속 진행되지만 게이지만 체력 0의 검은 상태로 동결됩니다.
